...
bool canReachFinalConfiguration(string& s) {
    if (count(s.begin(), s.end(), '1') % 2 != 0) return false;
    else{
        s ='0'+s+'0';
        int n = s.size();
        int cnt = 0;
        ...
        if(cnt == 1 && count(s.begin(), s.end(), '1') == 2) return false;
    }
    return true;
}
...
int main() {
    ...
    REP(i, t){
        ...
        if (canReachFinalConfiguration(s)) {
            cout << "YES" << endl;
        } else {
            cout << "NO" << endl;
        }
