...
bool who_win(int64_t x, int64_t y, int64_t z, int64_t w){
    if(x > z && y > w){ //2連勝
        return true;
    }
    if(x > z && y == w){ //1勝1敗
...
int main() {
    ...
    while(t--){
        ...
        cin >> a1 >> a2 >> b1 >> b2;
        int64_t ans = 0;
        if(who_win(a1, a2, b1, b2)) ans++;
        if(who_win(a2, a1, b1, b2)) ans++;
        if(who_win(a1, a2, b2, b1)) ans++;
        if(who_win(a2, a1, b2, b1)) ans++;
        cout << ans << endl;
    }
    return 0;
