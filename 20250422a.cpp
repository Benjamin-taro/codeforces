...
int main() {
    ...
    while(t--){
        ...
        vector<pair<int64_t, int64_t>> vec(freq.begin(), freq.end());

        sort(vec.begin(), vec.end(), [](const auto &a, const auto &b) {
            return a.second > b.second;
        }); 

        vector<int64_t> mx_ans;
        int64_t negcnt = 0;
        int64_t cnt = 1;
        for(auto [i, j] : vec){
            mx_ans.push_back(cnt);
            if(j == 2) negcnt++;
            cnt++;
        }
        cnt--;
        for(int64_t i = 0; i < negcnt; i++){
            mx_ans.push_back(cnt - 1);
            cnt--;
        }
