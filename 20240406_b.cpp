...
int main()
{
	...
	for(;T--;)
	{
		...
		vector<int>idx;
		for(int i=0;i<N;i++)if(A[i]>A[K-1])idx.push_back(i);
		if(idx.empty())cout<<N-1<<"\n";
		else if(idx[0]>K-1)cout<<idx[0]-1<<"\n";
		else
		{
			int ans=idx[0]-1;
			int f=idx[0]>0;
			if(idx.size()==1)
			{
				ans=max(ans,K-1-idx[0]-1+f);
			}
			else
			{
				if(idx[1]<K-1)ans=max(ans,idx[1]-idx[0]-1+f);
				else ans=max(ans,K-1-idx[0]-1+f);
			}
			cout<<ans<<"\n";
		}
	}
