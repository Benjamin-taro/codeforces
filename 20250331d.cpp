...
int main(){
    ...
    while(t--){
        ...
        vector<ll> L(n), R(n);
        for (int j = 0; j < n; j++){
            L[j] = llabs(A_rev[j] - b[j]);
            R[j] = llabs(A_rev[j] - b[m - n + j]);
        }
        
        vector<ll> prefixL(n+1, 0), suffixR(n+1, 0);
        for (int j = 0; j < n; j++){
            prefixL[j+1] = prefixL[j] + L[j];
        }
        for (int j = n-1; j >= 0; j--){
            suffixR[j] = suffixR[j+1] + R[j];
        }
        
        ll ans = 0;
        for (int x = 0; x <= n; x++){
            ll cur = prefixL[x] + suffixR[x];
            ans = max(ans, cur);
        }
        cout << ans << "\n";
    }
