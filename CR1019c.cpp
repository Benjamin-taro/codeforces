...
int main() {
    ...
    while(t--){
        ...
        int64_t posmin = INT_MAX;
        bool ans = false;
        for(int64_t r = 2; r < n; r++){
            minv = min(minv, prefix[r-1]);
            maxv = max(maxv, prefix[r-1]);
            if(prefix[r-1] >=0) posmin = min(posmin, prefix[r-1]);
            if(prefix[n]-prefix[r] >= 0){ //右端がクリア
                if(maxv >= 0 || prefix[r] - minv >= 0){ //片方または両方がクリアしていればいい
                    ans = true;
                    break;
                }
            }
            else{ //右側クリアしてない
                if(posmin >= 0 && prefix[r] - posmin >= 0){ //左二つがクリアしないといけない
                    ans = true;
                    break;
                }
            }
