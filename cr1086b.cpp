...
int main() {
    ...
    while(t--){
        ...
        int64_t first_clear = 0;
        if(p > k){
            vector<int64_t> before(a.begin(), a.begin() + p - 1); 
            sort(before.begin(), before.end());
            for(int i = 0; i < p - k; i++){
                first_clear += before[i];
            }
        }

        vector<int64_t> others;
        REP(i, n) if(i != p-1) others.push_back(a[i]);
        sort(others.begin(), others.end());
        int64_t cycle_clear = 0;
        for(int i = 0; i < n - k; i++){
            cycle_clear += others[i];
        }

        int64_t ans = 0;

        int64_t cost1 = first_clear + win;
        if(m >= cost1){
            m -= cost1;
            ans = 1;
        }
        ...
        if(ans > 0){
            int64_t cycle_cost = cycle_clear + win;
            if(cycle_cost > 0){
                ans += m / cycle_cost;
            }
        }
