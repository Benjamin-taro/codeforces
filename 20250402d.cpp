...
int main(){
    ...
    while(t--){
        ...
        bool beforemidhasZero = false;
        bool aftermidhasZero = false;
        int64_t mid = n / 2;
        int64_t right = n;
        for (int i = 0; i < n; i++){
            cin >> a[i];
            if(i < mid && a[i] == 0){
                beforemidhasZero = true;
            }
            else if(i >= mid && a[i] == 0){
                aftermidhasZero = true;
            }
        }

        vector<pair<int64_t, int64_t>> ans;
        // Python版では a[:mid] に 0 があれば [1, mid] とし、r -= mid-1 としている
                // Python版では a[mid:] に 0 があれば [mid+1, n] とし、r -= n-mid-1 としている
        if(aftermidhasZero){
        ...
        if(beforemidhasZero){
            ans.push_back({1, mid});
            right -= (mid - 1);  // 修正: mid ではなく mid-1 を引く
        }
